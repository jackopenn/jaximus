...
def train(cfg):
    ...
    batch = (
        jax.random.randint(0, (cfg.data.batch_size, cfg.data.max_length), dtype=jnp.int32, out_shardings=logical_to_physical(("batch", "seq"))),
        jax.random.randint(1, (cfg.data.batch_size, cfg.data.max_length), dtype=jnp.int32, out_shardings=logical_to_physical(("batch", "seq")))
    )
    micro_step = 0
    while True:
