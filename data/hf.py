...
#         .to_iter_dataset(read_options=grain.ReadOptions(num_threads=1, prefetch_buffer_size=64*1024))
...
def get_hf_dataset(
    ...
    def tokenize_and_group_texts(texts):
        ...
        # Prepend BOS token to each document before grouping (acts as delimiter)
        if hasattr(tokenizer, 'bos_token_id') and tokenizer.bos_token_id is not None:
            bos_token_id = tokenizer.bos_token_id
            for k in tokenized_batch.keys():
                if k == 'input_ids':
                    # Prepend BOS token to each document in the batch
                    tokenized_batch[k] = [
                        np.concatenate([[bos_token_id], doc]) for doc in tokenized_batch[k]
                    ]
