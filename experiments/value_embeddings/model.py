...
class LayerWeights:
    ...
    value_embedding_weights: Optional[ValueEmbeddingWeights] = None
...
def _should_use_value_embeddings(config, layer_idx):
    if config.value_embeddings is None:
        return False
    if layer_idx == 0:
...
def _init_value_embedding_weights(config, layer_idx):
    if not _should_use_value_embeddings(config, layer_idx):
        return None
    if config.value_embeddings != "learnt":
        return None
    return ValueEmbeddingWeights(lambda_v=jnp.array(0.5, dtype=jnp.float32))
...
def _init_layer_weights(config, layer_idx, keys):
    return LayerWeights(
        ...
        value_embedding_weights=_init_value_embedding_weights(config, layer_idx),
    )
...
def _attention_with_v(
    ...
    value_embedding_weights: Optional[ValueEmbeddingWeights],
    value_embeddings_mode: Optional[str],
    mask: Optional[jax.Array] = None,
) -> Tuple[jax.Array, jax.Array]:
    """Attention that returns both output and v (for v1 capture). Supports value residual mixing."""
    dtype = jnp.bfloat16
    ...
    if v1 is not None and value_embeddings_mode is not None:
        with jax.named_scope("value_residual"):
            if value_embeddings_mode == "fixed":
                v = 0.5 * v + 0.5 * v1
            elif value_embeddings_mode == "learnt":
                lam = value_embedding_weights.lambda_v.astype(dtype)
                v = lam * v + (1.0 - lam) * v1
...
def model_forward(x, weights, config, rope_cos=None, rope_sin=None, mask=None):
    ...
    for layer_idx, layer_weights in enumerate(weights.layer_weights):
        ...
        value_emb_mode = config.value_embeddings if use_value_emb else None
        ...
        x, v = _attention_with_v(
            ...
            v1=v1,
            value_embedding_weights=layer_weights.value_embedding_weights,
            value_embeddings_mode=value_emb_mode,
            mask=mask,
        )
        x = x + residual
