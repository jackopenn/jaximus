...
def _get_optimizer_group(path):
    """Classify weight path into optimizer group: embed (adam) or other (muon)."""
    names = [getattr(p, "name", None) for p in path]
    if names[0] in ("embed", "unembed", "engram"):
        return "embed"
    return "other"
        ...
        optax.partition(
            {
                "embed": optax.adamw(
                    learning_rate=warmup_stable_decay_schedule(
                        opt.peak_lr, opt.warmup_steps, opt.decay_steps, cfg.max_steps
                    ),
