...
def generate(model, tokenizer, prompts, max_length, n_samples, top_k, temperature):
    ...
    prompts_tokens = tokenizer.batch_encode_plus(generate_config.prompts)['input_ids']
    ...
    sample_length = generate_config.max_length - max(prompt_token_lengths)

    prompts_tokens = [jnp.pad(x, (generate_config.max_length - x.shape[0], 0), mode='constant', constant_values=tokenizer.pad_token_id) for x in prompts_tokens]
    prompts_tokens = [jnp.repeat(x[jnp.newaxis, :], generate_config.n_samples, axis=0) for x in prompts_tokens]
    prompts_tokens = jnp.concatenate(prompts_tokens, axis=0)
    # print(prompts_tokens)
    mask = prompts_tokens != tokenizer.pad_token_id
    generated_tokens = sample(model, prompts_tokens, mask, sample_length, generate_config.top_k, generate_config.temperature)
    ...
    samples = {}
    for i in range(0, generate_config.n_samples * len(generate_config.prompts), generate_config.n_samples):
        prompt_idx = i // generate_config.n_samples
        # print(f"prompt: {generate_config.prompts[prompt_idx]}, (prompt token length: {prompt_token_lengths[prompt_idx]})")
        for j in range(generate_config.n_samples):
            # print(f"sample {j}: {decoded_tokens[i + j]}")
            samples[generate_config.prompts[prompt_idx]] = samples.get(generate_config.prompts[prompt_idx], []) + [decoded_tokens[i + j]]
        # print()
